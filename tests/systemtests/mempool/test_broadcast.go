...
// TestRunTxBroadcasting tests transaction broadcasting and duplicate handling in a multi-node network.
//
...
func TestRunTxBroadcasting(t *testing.T) {
	testCases := []struct {
		name    string
		actions []func(TestSuite)
	}{
		{
			...
			actions: []func(TestSuite){
				...
				func(s TestSuite) {
					...
					tx3, err := s.SendTx(t, s.Node(1), "acc0", 2, s.GetTxGasPrice(s.BaseFee()), nil)
					...
					tx2, err := s.SendTx(t, s.Node(2), "acc0", 1, s.GetTxGasPrice(s.BaseFee()), nil)
		...
		},
		{
			...
			actions: []func(TestSuite){
				func(s TestSuite) {
					...
					_, err = s.SendTx(t, s.Node(1), "acc0", 0, s.GetTxGasPrice(s.BaseFee()), nil)
